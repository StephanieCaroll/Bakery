      ...
      <main className="flex-1 bg-[#f4a28c] lg:rounded-l-[5rem] overflow-y-auto order-1 lg:order-2 shadow-2xl h-full p-6 lg:p-12 no-scrollbar">
        {!selectedProduct ? (
          <div className="max-w-5xl mx-auto py-4">
            <button onClick={() => navigate('/')} className="flex items-center gap-2 text-[#bc232d] font-black uppercase tracking-widest text-sm mb-10 hover:translate-x-[-5px] transition-transform">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
                {favorites.map((item) => (
                  <div key={item.favId} onClick={() => setSelectedProduct(item)} className="bg-white/50 backdrop-blur-md p-4 rounded-[2.5rem] border border-white/20 shadow-lg flex items-center gap-5 cursor-pointer hover:scale-[1.02] transition-all group min-h-[140px]">
                    ...
                    <div className="flex-1 flex flex-col justify-between py-1 overflow-hidden">
                      <div>
                        <h3 className="text-[#bc232d] font-black text-xl leading-tight uppercase tracking-tighter mb-1 break-words">{item.name}</h3>
                        <p className="text-[#bc232d] font-black text-lg opacity-80 leading-none">R${item.price}</p>
                      </div>
                      <div className="flex gap-3 mt-3">
                        <button className="bg-[#bc232d] text-white px-5 py-2.5 rounded-2xl shadow-md hover:brightness-110 active:scale-95 transition flex-1 font-black text-xs uppercase tracking-widest">Pedir</button>
                        <button onClick={(e) => removeFavorite(e, item.favId)} className="bg-white/70 text-[#bc232d] p-2.5 rounded-2xl shadow-sm hover:bg-white hover:text-red-600 active:scale-90 transition"><Trash2 size={18}/></button>
                      </div>
                    </div>
                  </div>
                ))}
